...
from ..model.value_objects import (
    GaitPattern, MovementTrajectory, MotionCommand, MotionType,
    LocomotionSkill, SkillType, MasteryLevel
)
...
class MovementQualityAnalyzer:
    ...
    def assess_energy_efficiency(self, joint_commands: List[np.ndarray], 
                               ...
                               time_elapsed: float) -> Tuple[float, str]:
        """
        ...
        if total_energy == 0:
            ...
            assessment = self._generate_efficiency_assessment(efficiency_score, distance_covered, time_elapsed)
        
        return efficiency_score, assessment
    
    def detect_movement_patterns(self, state_history: List[RobotState]) -> MovementPatternAnalysis:
        """
    ...
    def evaluate_skill_progression(self, skill_type: SkillType, 
                                 recent_trajectories: List[MovementTrajectory]) -> Dict[str, Any]:
        """
        ...
        mastery_indicators = self._identify_mastery_indicators(skill_type, skill_metrics)
    ...
    def _extract_gait_pattern(self, trajectory: MovementTrajectory) -> GaitPattern:
        ...
        estimated_stride_frequency = max(avg_velocity / 0.5, 0.1)  # Assume 0.5m stride length
        estimated_stride_length = avg_velocity / estimated_stride_frequency if estimated_stride_frequency > 0 else 0.0
    ...
    def _calculate_stability_score(self, trajectory: MovementTrajectory, 
                                 gait_pattern: GaitPattern) -> float:
        ...
        gait_stability = min(gait_pattern.stability_margin / 0.1, 1.0)  # Normalize to 0.1m
    ...
    def _assess_stability(self, stability_score: float, gait_pattern: GaitPattern) -> str:
        """Generate stability assessment text."""
        if stability_score >= 0.8:
            return f"Excellent stability with {gait_pattern.stability_margin:.3f}m margin"
    ...
    def _assess_efficiency(self, efficiency_score: float, gait_pattern: GaitPattern) -> str:
        """Generate efficiency assessment text."""
        if efficiency_score >= 0.8:
            return f"Highly efficient gait at {gait_pattern.get_walking_speed():.2f}m/s"
        elif efficiency_score >= 0.6:
            return f"Good efficiency, walking at {gait_pattern.get_walking_speed():.2f}m/s"
        elif efficiency_score >= 0.4:
            return f"Moderate efficiency, room for improvement"
        else:
            return f"Low efficiency, significant energy waste detected"
    
    def _generate_gait_recommendations(self, gait_pattern: GaitPattern, 
                                     stability_score: float,
                                     efficiency_score: float,
                                     symmetry_score: float) -> List[str]:
        """Generate recommendations for gait improvement."""
        recommendations = []
        
        if stability_score < 0.6:
            recommendations.append("Focus on balance training to improve stability margin")
            recommendations.append("Reduce walking speed to improve stability")
        
        if efficiency_score < 0.6:
            recommendations.append("Work on smoother joint movements to improve efficiency")
        ...
        if not recommendations:
            recommendations.append("Continue current training to maintain good gait quality")
        
        return recommendations
    
    def _generate_efficiency_assessment(self, efficiency_score: float, 
                                      distance: float, time: float) -> str:
        """Generate efficiency assessment text."""
        speed = distance / time
    ...
    def _identify_movement_strengths(self, state_history: List[RobotState]) -> List[str]:
    ...
    def _calculate_skill_metrics(self, skill_type: SkillType, 
                               trajectories: List[MovementTrajectory]) -> Dict[str, float]:
        ...
        avg_velocity = sum(traj.get_average_velocity() for traj in trajectories) / len(trajectories)
        avg_smoothness = sum(traj.get_smoothness_score() for traj in trajectories) / len(trajectories)
        ...
        if skill_type == SkillType.FORWARD_WALKING:
            ...
            metrics['overall_score'] = (avg_smoothness + forward_consistency + min(avg_velocity / 1.0, 1.0)) / 3.0
    ...
    def _identify_mastery_indicators(self, skill_type: SkillType, 
                                   skill_metrics: Dict[str, float]) -> List[str]:
    ...
    def _determine_next_milestones(self, skill_type: SkillType, 
                                 skill_metrics: Dict[str, float]) -> List[str]:
    ...
    def _calculate_forward_consistency(self, trajectories: List[MovementTrajectory]) -> float:
    ...
    def _calculate_turning_quality(self, trajectories: List[MovementTrajectory]) -> float:
