...
class LearningEpisode:
    ...
    def start_episode(self, target_skill: Optional[SkillType] = None) -> None:
        ...
        logger.info(f"Started episode {self.episode_id.value} targeting skill {target_skill}")
    ...
    def get_complexity_score(self) -> float:
        ...
        total_complexity = sum(cmd.get_complexity_score() for cmd in self.motion_commands)
...
class CurriculumStage:
    ...
    def is_skill_mastered(self, skill: SkillType, 
                         threshold: MasteryLevel = MasteryLevel.INTERMEDIATE) -> bool:
        """Check if a skill is mastered at required level."""
        assessment = self.skill_assessments.get(skill)
        if assessment is None:
            return False
        
        return (assessment.skill.mastery_level.get_numeric_value() >= 
    ...
    def get_progress_percentage(self) -> float:
        ...
        episode_progress = min(
            self.episodes_completed / self.expected_duration_episodes, 1.0
        ) * 50.0  # Episodes contribute 50%
        
        success_progress = min(
            self.get_success_rate() / self.target_success_rate, 1.0
        ) * 30.0  # Success rate contributes 30%
        
        skill_progress = self._calculate_skill_progress() * 20.0  # Skills contribute 20%
    ...
    def get_remaining_requirements(self) -> Dict[str, Any]:
        ...
        current_success_rate = self.get_success_rate()
        if current_success_rate < self.target_success_rate:
            requirements['success_rate_gap'] = self.target_success_rate - current_success_rate
    ...
    def _evaluate_criteria(self, criteria: AdvancementCriteria, threshold: float) -> bool:
        ...
        if criteria == AdvancementCriteria.EPISODE_COUNT:
            ...
            mastered_count = sum(
                1 for skill in self.target_skills 
