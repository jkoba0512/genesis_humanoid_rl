...
class LearningSessionRepository(ABC):
    ...
    @abstractmethod
    def find_completed_sessions(self, 
                              start_date: Optional[datetime] = None,
                              end_date: Optional[datetime] = None) -> List[LearningSession]:
...
class HumanoidRobotRepository(ABC):
    ...
    @abstractmethod
    def find_by_skill_mastery(self, skill: SkillType, 
                            min_proficiency: float = 0.7) -> List[HumanoidRobot]:
