...
class SimpleTerminationChecker:
    ...
    def should_terminate(self, robot_state: RobotState, 
...
class HumanoidWalkingEnvV2(Env):
    ...
    def __init__(
        ...
        self.observations = observation_manager or container.get(ObservationManagerProtocol)
        ...
        logger.info(f"Initialized HumanoidWalkingEnvV2 with {self.config.num_joints} joints")
    ...
    def reset(self, seed: Optional[int] = None, 
        ...
        try:
            ...
            observation = self.observations.get_observation(self.robot_state, obs_context)
            ...
            logger.debug(f"Environment reset completed, observation shape: {observation.shape}")
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        try:
            ...
            terminated = self.termination_checker.should_terminate(
                self.robot_state, {}
            )
            ...
            if terminated:
                info["termination_reason"] = self.termination_checker.get_termination_reason()
