...
class CurriculumHumanoidEnv(Env):
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[Dict] = None) -> Tuple[np.ndarray, Dict]:
        ...
        if hasattr(self, 'current_episode_reward'):
            stage_advanced, new_stage = self.curriculum.update_episode(self.current_episode_reward)
            if stage_advanced:
        ...
        self.robot = self.scene.add_entity(
            gs.morphs.URDF(
                file=urdf_path,
                pos=(initial_variation[0], initial_variation[1], 1.0 + initial_variation[2]),
        ...
        if self.robot is not None:
            ...
            if actual_dofs != self.num_joints:
                print(f"Warning: Robot has {actual_dofs} DOFs, expected {self.num_joints}")
