...
class HumanoidWalkingEnv(Env):
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        if self.robot is not None:
            ...
            target_pos = current_pos + torch.tensor(action * 0.1, dtype=current_pos.dtype, device=current_pos.device)  # Small position changes
    ...
    def _calculate_reward(self, action: np.ndarray) -> float:
        ...
        if hasattr(self, '_previous_pos'):
            ...
            velocity_reward = min(forward_velocity / self.target_velocity, 2.0)  # Cap at 2x target
        ...
        if quat_norm > 0:
            tilt_magnitude = np.sqrt(quat[0]**2 + quat[1]**2)  # x,y components indicate tilt
            stability_reward = max(0.0, 1.0 - 2.0 * tilt_magnitude)  # Less tilt = higher reward
        else:
