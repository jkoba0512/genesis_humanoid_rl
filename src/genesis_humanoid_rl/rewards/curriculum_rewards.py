...
class CurriculumRewardCalculator:
    ...
    def calculate_reward(self, robot, action: np.ndarray, info: Dict[str, Any] = None) -> float:
    ...
    def _calculate_smoothness_penalty(self, action: np.ndarray, info: Dict[str, Any]) -> float:
    ...
    def _calculate_stage_specific_reward(self, pos: np.ndarray, quat: np.ndarray, 
                                       action: np.ndarray, info: Dict[str, Any]) -> float:
    ...
    def _speed_control_reward(self, pos: np.ndarray) -> float:
        ...
        if self.previous_pos is not None:
            ...
            velocity_precision = 1.0 - abs(actual_velocity - target_velocity) / max(target_velocity, 0.1)
