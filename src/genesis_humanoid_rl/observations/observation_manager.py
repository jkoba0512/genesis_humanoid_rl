...
class HumanoidObservationManager(BaseObservationManager):
    ...
    def get_observation(self, robot_state: RobotState, 
                       context: ObservationContext) -> np.ndarray:
        """
    ...
    def _normalize_observation(self, observation: np.ndarray) -> np.ndarray:
        ...
        normalized[pos_start:pos_end] = 2.0 * (
            (observation[pos_start:pos_end] - pos_min) / (pos_max - pos_min)
        ) - 1.0
        ...
        normalized[joint_pos_start:joint_pos_end] = 2.0 * (
            (observation[joint_pos_start:joint_pos_end] - joint_min) / (joint_max - joint_min)
        ) - 1.0
        
        # Normalize joint velocities
        vel_min, vel_max = ranges['joint_velocities']
        normalized[joint_vel_start:joint_vel_end] = 2.0 * (
            (observation[joint_vel_start:joint_vel_end] - vel_min) / (vel_max - vel_min)
        ) - 1.0
        ...
        normalized[target_vel_idx] = 2.0 * (
            (target_vel - target_min) / (target_max - target_min)
        ) - 1.0
    ...
    def _validate_robot_state(self, robot_state: RobotState) -> None:
        ...
        if robot_state.orientation.shape != (4,):
            raise ValueError(f"Invalid orientation shape: {robot_state.orientation.shape}")
        
        if robot_state.joint_positions.shape != (self.num_joints,):
            raise ValueError(f"Invalid joint positions shape: {robot_state.joint_positions.shape}")
        
        if robot_state.joint_velocities.shape != (self.num_joints,):
            raise ValueError(f"Invalid joint velocities shape: {robot_state.joint_velocities.shape}")
    ...
    def _validate_context(self, context: ObservationContext) -> None:
        ...
        if context.previous_action.shape != (self.num_joints,):
            raise ValueError(f"Invalid previous action shape: {context.previous_action.shape}")
        
        if not isinstance(context.target_velocity, (int, float)):
            raise TypeError(f"Target velocity must be numeric, got {type(context.target_velocity)}")
    ...
    def get_observation_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'components': {
                ...
                'target_velocity': (7 + 3 * self.num_joints, 7 + 3 * self.num_joints + 1),
            }
        }
