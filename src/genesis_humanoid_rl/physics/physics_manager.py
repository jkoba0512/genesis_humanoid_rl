...
class GenesisPhysicsManager(BasePhysicsManager):
    ...
    def __init__(self, robot_grounding_factory=None):
        super().__init__()
        self.robot_grounding_factory = robot_grounding_factory or RobotGroundingFactory()
    ...
    def add_robot(self, urdf_path: str, position: np.ndarray) -> Any:
        ...
        if self.robot is not None:
            try:
                ...
                grounding_height = grounding_calculator.get_grounding_height(safety_margin=0.03)
                self.robot.set_pos(torch.tensor([position[0], position[1], grounding_height]))
        ...
        logger.info(f"Robot loaded with {self.robot.n_dofs} DOFs and {self.robot.n_links} links")
        return self.robot
