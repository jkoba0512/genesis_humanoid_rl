...
class SafeJSONHandler:
    ...
    def loads(self, json_string: str, field_name: str = "json_data") -> Union[Dict, List, Any]:
        ...
        if not isinstance(json_string, str):
            raise JSONSecurityError(f"{field_name} must be a string, got {type(json_string)}")
        
        # Check size limit
        if len(json_string) > self.max_size:
            logger.warning(f"JSON payload too large: {len(json_string)} bytes > {self.max_size}")
        ...
        if json_string.count('{') > 1000 or json_string.count('[') > 1000:
            ...
            raise JSONSecurityError(f"{field_name} contains excessive nesting structures")
    ...
    def _validate_parsed_data(self, data: Any, field_name: str, current_depth: int = 0) -> None:
        ...
        if current_depth > self.max_depth:
            logger.warning(f"JSON depth limit exceeded: {current_depth} > {self.max_depth}")
            raise JSONSecurityError(f"{field_name} exceeds maximum nesting depth of {self.max_depth}")
        
        if isinstance(data, dict):
            # Validate dictionary
            if len(data) > 1000:  # Prevent huge objects
                raise JSONSecurityError(f"{field_name} contains too many keys: {len(data)}")
            
            for key, value in data.items():
                # Validate key
                if not isinstance(key, str):
                    raise JSONSecurityError(f"{field_name} contains non-string key: {type(key)}")
                
                if len(key) > 100:  # Reasonable key length limit
                    raise JSONSecurityError(f"{field_name} contains key that is too long: {len(key)} chars")
                
                # Recursively validate value
                self._validate_parsed_data(value, f"{field_name}.{key}", current_depth + 1)
        
        elif isinstance(data, list):
            # Validate array
            if len(data) > 10000:  # Prevent huge arrays
                raise JSONSecurityError(f"{field_name} contains too many items: {len(data)}")
            
            for i, item in enumerate(data):
                # Recursively validate item
                self._validate_parsed_data(item, f"{field_name}[{i}]", current_depth + 1)
            ...
            if isinstance(data, float):
                if abs(data) > 1e15:  # Prevent extremely large numbers
                    raise JSONSecurityError(f"{field_name} numeric value too large: {data}")
        ...
        # Allow None, bool, and validated types
            ...
            raise JSONSecurityError(f"{field_name} contains unsupported data type: {type(data)}")
...
def safe_loads(json_string: str, field_name: str = "json_data") -> Union[Dict, List, Any]:
