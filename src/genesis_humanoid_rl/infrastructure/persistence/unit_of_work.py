    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self._transaction_active:
            if exc_type is not None:
                logger.warning(f"Transaction rolling back due to exception: {exc_type.__name__}: {exc_val}")
                self.rollback()
            else:
                try:
                    self.commit()
...
@contextmanager
def unit_of_work(db_connection: DatabaseConnection) -> Generator[SQLiteUnitOfWork, None, None]:
