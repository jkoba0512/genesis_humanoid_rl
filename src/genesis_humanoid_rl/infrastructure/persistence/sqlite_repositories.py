...
security_validator = SecurityValidator(max_string_length=10000, max_json_size=1000000)  # 1MB limit
...
class SQLiteLearningSessionRepository(LearningSessionRepository):
            ...
            if episode.motion_commands:
                ...
                try:
                    motion_command_json = safe_dumps({
                        ...
                        "velocity": security_validator.validate_velocity(motion_command.velocity),
    ...
    def find_by_id(self, session_id: SessionId) -> Optional[LearningSession]:
        ...
        for row in episode_rows:
            try:
                ...
                velocity = security_validator.validate_velocity(command_data["velocity"])
    ...
    def find_completed_sessions(self, start_date=None, end_date=None) -> List[LearningSession]:
...
class SQLiteHumanoidRobotRepository(HumanoidRobotRepository):
    ...
    def _save_robot_data(self, conn, robot: HumanoidRobot) -> None:
        ...
        try:
            config_json = safe_dumps({
                "robot_type": security_validator.validate_string_input(robot.robot_type.value, "robot_type"),
                "joint_count": security_validator.validate_numeric(robot.joint_count, "joint_count", 1, 100),
                "height": security_validator.validate_numeric(robot.height, "height", 0.1, 3.0),
                "created_at": robot.created_at.isoformat() if hasattr(robot.created_at, 'isoformat') else str(robot.created_at)
            })
        except (JSONSecurityError, ValidationError) as e:
        ...
        try:
            ...
            for skill_type, skill in robot.learned_skills.items():
                learned_skills_data[skill_type.value] = {
                    "skill_type": security_validator.validate_skill_type(skill.skill_type.value),
                    "mastery_level": security_validator.validate_string_input(skill.mastery_level.value, "mastery_level"),
                    "proficiency_score": security_validator.validate_proficiency_score(skill.proficiency_score),
                    "last_assessed": skill.last_assessed.isoformat() if skill.last_assessed else None
                }
        ...
        try:
            ...
            for assessment in robot.skill_history:
                skill_history_data.append({
                    "skill": {
                        "skill_type": security_validator.validate_skill_type(assessment.skill.skill_type.value),
                        "mastery_level": security_validator.validate_string_input(assessment.skill.mastery_level.value, "mastery_level"),
                        "proficiency_score": security_validator.validate_proficiency_score(assessment.skill.proficiency_score),
                        "last_assessed": assessment.skill.last_assessed.isoformat() if assessment.skill.last_assessed else None
                    },
                    "assessment_score": security_validator.validate_proficiency_score(assessment.assessment_score),
                    "confidence_level": security_validator.validate_confidence_level(assessment.confidence_level),
                    "evidence_quality": security_validator.validate_confidence_level(assessment.evidence_quality)
                })
        ...
        try:
            ...
            for metrics in robot.performance_history:
                ...
                performance_history_data.append({
                    "success_rate": security_validator.validate_proficiency_score(metrics.success_rate),
                    "average_reward": security_validator.validate_numeric(metrics.average_reward, "average_reward", -100, 100),
                    "learning_progress": security_validator.validate_numeric(metrics.learning_progress, "learning_progress", -1, 1),
                    "skill_scores": {k.value: security_validator.validate_proficiency_score(v) for k, v in metrics.skill_scores.items()},
                    "gait_quality": security_validator.validate_proficiency_score(metrics.gait_quality)
                })
    ...
    def find_by_id(self, robot_id: RobotId) -> Optional[HumanoidRobot]:
        """Find a robot by ID."""
        cursor = self.db.execute(
            "SELECT * FROM humanoid_robots WHERE id = :id",
            {"id": robot_id.value}
        )
        row = cursor.fetchone()
    ...
    def find_all(self) -> List[HumanoidRobot]:
        """Find all robots."""
        cursor = self.db.execute("SELECT id FROM humanoid_robots ORDER BY created_at DESC")
    ...
    def find_by_skill_level(self, skill_type: SkillType, min_proficiency: float) -> List[HumanoidRobot]:
        ...
        return [
            robot for robot in all_robots
            if skill_type in robot.learned_skills
            and robot.learned_skills[skill_type].proficiency_score >= min_proficiency
        ]
    
    def find_by_skill_mastery(self, skill: SkillType, min_proficiency: float = 0.7) -> List[HumanoidRobot]:
...
class SQLiteCurriculumPlanRepository(CurriculumPlanRepository):
    ...
    def _save_plan_data(self, conn, plan: CurriculumPlan) -> None:
        ...
        stages_json = safe_dumps([
            {
                ...
                "prerequisite_skills": [skill.value for skill in stage.prerequisite_skills],
        ...
        metadata_json = safe_dumps({
            "robot_type": plan.robot_type.value if hasattr(plan, 'robot_type') else "humanoid",
    ...
    def find_by_id(self, plan_id) -> Optional[CurriculumPlan]:
        ...
        cursor = self.db.execute(
            "SELECT * FROM curriculum_plans WHERE id = :id",
            {"id": plan_id_str}
        )
        row = cursor.fetchone()
        ...
        for stage_data in stages_data:
            ...
            prerequisite_skills = {SkillType(skill) for skill in stage_data.get("prerequisite_skills", [])}
            advancement_criteria = {
    ...
    def find_by_name(self, name: str) -> Optional[CurriculumPlan]:
        ...
        cursor = self.db.execute(
            "SELECT id FROM curriculum_plans WHERE name = :name",
            {"name": name}
        )
        row = cursor.fetchone()
