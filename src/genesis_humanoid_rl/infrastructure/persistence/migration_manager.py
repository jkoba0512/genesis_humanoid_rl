...
class DatabaseMigrationManager:
    ...
    @contextmanager
    def migration_transaction(self, version: str, name: str) -> Generator[sqlite3.Connection, None, None]:
        ...
        with self.db_connection.transaction() as conn:
            try:
                ...
                logger.info(f"Migration {version} '{name}' completed successfully in {execution_time:.2f}s")
    ...
    def _backup_existing_data(self, conn: sqlite3.Connection) -> Dict[str, List[sqlite3.Row]]:
        ...
        for table_name in existing_tables:
            if table_name != 'schema_migrations':
                ...
                logger.debug(f"Backed up {len(backup_data[table_name])} rows from {table_name}")
    ...
    def _create_optimized_schema(self, conn: sqlite3.Connection) -> None:
        ...
        for i, statement in enumerate(migration_statements):
            try:
                ...
                logger.debug(f"Executed migration statement {i+1}/{len(migration_statements)}")
            except Exception as e:
                logger.error(f"Failed to execute statement {i+1}: {statement}")
                raise
        
        logger.info(f"Created optimized schema with {len(migration_statements)} statements")
    
    def _migrate_data_to_new_schema(self, conn: sqlite3.Connection, backup_data: Dict[str, List[sqlite3.Row]]) -> None:
    ...
    def _migrate_robots_data(self, conn: sqlite3.Connection, robots_data: List[sqlite3.Row]) -> None:
        """Migrate robots data to normalized structure."""
        logger.debug("Migrating robots data...")
        
        for robot_row in robots_data:
            # Parse stored JSON data
            learned_skills = json.loads(robot_row['learned_skills']) if robot_row['learned_skills'] else {}
            ...
            for skill_type, skill_data in learned_skills.items():
                if isinstance(skill_data, dict):
                    ...
                    """, (
                        skill_type,
                        skill_type.replace('_', ' ').title(),  # Convert FORWARD_WALKING to Forward Walking
                        f"Migrated skill: {skill_type}",
                        'locomotion',  # Default category
                        1.0  # Default difficulty
                    ))
                    
                    proficiency = skill_data.get('proficiency_score', 0.0) if isinstance(skill_data, dict) else 0.0
                    mastery_level = skill_data.get('mastery_level', 'BEGINNER') if isinstance(skill_data, dict) else 'BEGINNER'
    ...
    def _migrate_curriculum_plans_data(self, conn: sqlite3.Connection, plans_data: List[sqlite3.Row]) -> None:
    ...
    def _migrate_learning_sessions_data(self, conn: sqlite3.Connection, sessions_data: List[sqlite3.Row]) -> None:
    ...
    def _migrate_learning_episodes_data(self, conn: sqlite3.Connection, episodes_data: List[sqlite3.Row]) -> None:
        ...
        for episode_row in episodes_data:
            ...
            success = episode_row['success'] if 'success' in episode_row.keys() else False
    ...
    def _migrate_domain_events_data(self, conn: sqlite3.Connection, events_data: List[sqlite3.Row]) -> None:
    ...
    def _validate_migration(self, conn: sqlite3.Connection, original_data: Dict[str, List[sqlite3.Row]]) -> None:
