...
from .genesis_monitor import GenesisAPIMonitor, GenesisCompatibilityReport, GenesisCompatibilityLevel
...
class VersionTestSuite:
    ...
    def get_pass_rate(self) -> float:
        ...
        passed_tests = sum(1 for test in self.tests 
                          if test.actual_result == VersionTestResult.PASS)
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        for result in VersionTestResult:
            results[result.value] = sum(1 for test in self.tests 
                                      if test.actual_result == result)
...
class GenesisVersionTester:
    ...
    def _initialize_test_suites(self) -> None:
        ...
        core_tests = [
            GenesisVersionTest(
                ...
                description="Test basic Genesis module import"
            ),
            GenesisVersionTest(
    ...
    async def test_version_compatibility(self, 
                                       ...
                                       test_suites: Optional[List[str]] = None) -> Dict[str, VersionTestSuite]:
        """Test Genesis version compatibility."""
        if test_suites is None:
            test_suites = list(self.test_suites.keys())
        
        self.logger.info(f"Testing Genesis compatibility for version: {version or 'current'}")
    ...
    async def _test_scene_creation(self, test: GenesisVersionTest) -> VersionTestResult:
        ...
        try:
            ...
            if successful_configs == len(configs):
                ...
                test.output = f"{successful_configs}/{len(configs)} configurations successful"
                return VersionTestResult.FAIL
            else:
    ...
    async def _test_rigid_body_simulation(self, test: GenesisVersionTest) -> VersionTestResult:
        ...
        try:
            ...
            test.output = f"Simulation FPS: {fps:.1f}, Avg step time: {avg_step_time:.4f}s"
    ...
    async def _test_observation_extraction(self, test: GenesisVersionTest) -> VersionTestResult:
    ...
    async def _test_simulation_performance(self, test: GenesisVersionTest) -> VersionTestResult:
        ...
        try:
            ...
            test.output = (f"Performance: {fps:.1f} FPS avg, "
    ...
    async def _test_headless_operation(self, test: GenesisVersionTest) -> VersionTestResult:
    ...
    async def _test_unitree_g1_loading(self, test: GenesisVersionTest) -> VersionTestResult:
        ...
        try:
            ...
            articulated_support = (hasattr(gs, 'RigidEntity') or 
                                 hasattr(gs, 'ArticulatedEntity') or
                                 hasattr(gs, 'Robot'))
    ...
    async def _test_bipedal_physics(self, test: GenesisVersionTest) -> VersionTestResult:
    ...
    async def _test_contact_detection(self, test: GenesisVersionTest) -> VersionTestResult:
    ...
    async def _test_balance_simulation(self, test: GenesisVersionTest) -> VersionTestResult:
    ...
    async def _test_single_robot_fps(self, test: GenesisVersionTest) -> VersionTestResult:
        """Test single robot simulation FPS."""
        return await self._test_simulation_performance(test)  # Same test
    
    async def _test_multi_robot_scaling(self, test: GenesisVersionTest) -> VersionTestResult:
        ...
        try:
            ...
            test.output = f"Multi-robot performance: {fps:.1f} FPS with {num_robots} robots"
    ...
    async def _test_memory_usage(self, test: GenesisVersionTest) -> VersionTestResult:
        ...
        try:
            ...
            if memory_increase < 100:  # Less than 100MB increase
                ...
                test.error_message = f"High memory usage increase: {memory_increase:.1f}MB"
                return VersionTestResult.FAIL
    ...
    async def _test_gpu_acceleration(self, test: GenesisVersionTest) -> VersionTestResult:
    ...
    async def _test_training_integration(self, test: GenesisVersionTest) -> VersionTestResult:
    ...
    async def _save_test_results(self, 
                                results: Dict[str, VersionTestSuite],
                                version: Optional[str]) -> None:
            ...
            for suite_name, suite in results.items():
                ...
                for test in suite.tests:
                    test_data = {
                        ...
                        'actual_result': test.actual_result.value if test.actual_result else None,
...
async def test_genesis_version(version: Optional[str] = None,
                              test_suites: Optional[List[str]] = None) -> Dict[str, VersionTestSuite]:
    """Test Genesis version compatibility."""
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        for suite_name, suite in results.items():
            ...
            # Show failed tests
            failed_tests = [test for test in suite.tests 
