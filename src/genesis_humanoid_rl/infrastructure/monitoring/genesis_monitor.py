...
class GenesisCompatibilityReport:
    ...
    def get_working_features(self) -> List[str]:
        ...
        return [test.feature_name for test in self.feature_tests 
...
class GenesisAPIMonitor:
    ...
    async def monitor_compatibility(self) -> GenesisCompatibilityReport:
        ...
        for feature_name, test_func in self.feature_tests.items():
            ...
            try:
                ...
                feature_results.append(GenesisFeatureTest(
    ...
    async def _run_feature_test(self, test_func, feature_name: str) -> GenesisFeatureTest:
    ...
    def _test_rigid_solver(self) -> GenesisFeatureTest:
        ...
        try:
            ...
            status = GenesisFeatureStatus.WORKING if solver_working else GenesisFeatureStatus.BROKEN
    ...
    def _test_entity_management(self) -> GenesisFeatureTest:
        ...
        try:
            ...
            # Determine status
            successful_tests = sum(1 for k, v in entity_tests.items() 
                                 if k.endswith('_creation') and v)
            total_tests = sum(1 for k in entity_tests.keys() 
                            if k.endswith('_creation'))
    ...
    def _assess_compatibility(self, feature_tests: List[GenesisFeatureTest]) -> GenesisCompatibilityLevel:
    ...
    def _generate_recommendations(self, 
                                feature_tests: List[GenesisFeatureTest],
                                version_info: GenesisVersionInfo) -> List[str]:
        """Generate recommendations based on test results."""
        recommendations = []
        
        # Version-specific recommendations
        if version_info.version == "unavailable":
            recommendations.append("Install Genesis physics engine: pip install genesis-world")
            return recommendations
        
        # Feature-specific recommendations
        broken_features = [test.feature_name for test in feature_tests 
                          if test.status == GenesisFeatureStatus.BROKEN]
        
        if "basic_import" in broken_features:
            recommendations.append("Genesis installation appears corrupted - reinstall Genesis")
        
        if "scene_creation" in broken_features:
            recommendations.append("Scene creation failing - check Genesis configuration and dependencies")
        
        if "rigid_solver" in broken_features:
            recommendations.append("Rigid body solver issues - may need Genesis version update")
        
        if "performance_baseline" in broken_features:
            recommendations.append("Poor performance detected - consider GPU acceleration or system upgrade")
        
        # Performance recommendations
        performance_tests = [test for test in feature_tests 
        ...
        if performance_tests:
            ...
            if fps < 10:
                recommendations.append("Performance is critically low - check system requirements")
        
        # GPU recommendations
        gpu_tests = [test for test in feature_tests if test.feature_name == "gpu_acceleration"]
        if gpu_tests and gpu_tests[0].status != GenesisFeatureStatus.WORKING:
            recommendations.append("GPU acceleration not available - training will be slower")
        
        # General recommendations
        working_count = sum(1 for test in feature_tests 
                          if test.status == GenesisFeatureStatus.WORKING)
        total_count = len(feature_tests)
        
        if working_count / total_count < 0.7:
            recommendations.append("Multiple features failing - consider Genesis version downgrade/upgrade")
        
        if not recommendations:
            recommendations.append("Genesis appears fully compatible - ready for production use")
...
async def monitor_genesis_compatibility(report_dir: str = "genesis_reports") -> GenesisCompatibilityReport:
