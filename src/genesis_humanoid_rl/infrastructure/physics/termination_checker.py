...
class TerminationChecker:
    ...
    def _check_extreme_positions(self, state: PhysicsState) -> TerminationCheckResult:
        ...
        if height < self.min_height:
            return TerminationCheckResult(
                ...
                confidence=1.0,
                diagnostic_data={'height': float(height), 'min_height': self.min_height},
                recovery_possible=True,
                severity='medium'
            )
        ...
        if height > self.max_height:
            return TerminationCheckResult(
                ...
                confidence=0.95,
                diagnostic_data={'height': float(height), 'max_height': self.max_height},
                recovery_possible=True,
                severity='medium'
            )
        ...
        if len(self.frame_times) > 10:
            ...
            if current_fps < self.min_fps:
                return TerminationCheckResult(
                    ...
                    confidence=0.7,
                    diagnostic_data={'current_fps': float(current_fps), 'min_fps': self.min_fps},
                    recovery_possible=True,
                    severity='medium'
                )
    ...
    def _check_timeout(self, state: PhysicsState) -> TerminationCheckResult:
        ...
        if elapsed_time > self.max_simulation_time:
            return TerminationCheckResult(
                ...
                confidence=1.0,
                diagnostic_data={'elapsed_time': elapsed_time, 'max_time': self.max_simulation_time},
                recovery_possible=False,
                severity='medium'
            )
    ...
    def _check_predictive_termination(self, state: PhysicsState) -> TerminationCheckResult:
        ...
        recent_velocities = [np.linalg.norm(s.robot_linear_velocity) for s in self.physics_history[-5:]]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'termination_counts': {k.value: v for k, v in self.termination_counts.items()},
