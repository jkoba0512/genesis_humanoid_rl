...
async def list_training_sessions(
    ...
    try:
        ...
        if robot_type:
            filtered_sessions = [s for s in filtered_sessions if s.robot_config.robot_type.value == robot_type]
        
        if search:
            search_lower = search.lower()
            filtered_sessions = [
                s for s in filtered_sessions
                if search_lower in s.session_name.lower() or
                   (s.description and search_lower in s.description.lower()) or
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to list training sessions: {str(e)}"
        )
...
async def control_training_session(
    ...
    try:
        ...
        logger.info(f"Training control action '{request.action}' for session {session_id}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to control training session: {str(e)}"
        )
...
async def simulate_training(session_id: str):
    ...
    try:
        ...
        while (session.status == TrainingStatus.RUNNING and 
            ...
            episode_reward = random.uniform(50, 150) + (current_timesteps / total_timesteps) * 100
            ...
            metric = TrainingMetrics(
                ...
                learning_rate=session.training_config.learning_rate * (0.99 ** (current_timesteps / 10000)),
            ...
            session.metrics.update({
                "best_reward": max(session.metrics.get("best_reward", 0), episode_reward),
                "average_reward": sum(m.episode_reward for m in training_metrics[session_id][-100:]) / min(100, len(training_metrics[session_id])),
                "training_fps": metric.fps,
                "estimated_time_remaining": (total_timesteps - current_timesteps) / 1000
            })
