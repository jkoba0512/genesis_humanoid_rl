    ...
    try:
        ...
        metrics = MonitoringMetrics(
            ...
            genesis_metrics={
                ...
                "gpu_acceleration": genesis_status.get('features', {}).get('gpu_acceleration', False)
            },
            api_metrics={
...
async def get_metrics_history(
    ...
    try:
        ...
        sampled_metrics = []
        if recent_metrics:
            sample_step = max(1, len(recent_metrics) // (hours * 60 // interval_minutes))
...
async def get_performance_report_by_id(report_id: str):
    ...
    if report_id not in performance_reports:
        raise HTTPException(
            status_code=404,
            detail=f"Performance report {report_id} not found"
        )
...
async def test_genesis_integration(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to start Genesis test: {str(e)}"
        )
...
async def get_genesis_compatibility():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to check Genesis compatibility: {str(e)}"
        )
...
async def run_genesis_tests():
    ...
    try:
        ...
        test_results = {
            ...
            "overall_status": "passed" if compatibility.get('compatible', False) else "failed"
        }
