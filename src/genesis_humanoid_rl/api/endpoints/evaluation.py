...
async def list_evaluations(
    ...
    try:
        ...
        if status:
            filtered_evaluations = [e for e in filtered_evaluations if e.status == status]
        
        if model_path:
            filtered_evaluations = [e for e in filtered_evaluations if model_path in e.model_path]
...
async def cancel_evaluation(evaluation_id: str):
    ...
    try:
        ...
        return BaseResponse(
            success=True,
            message=f"Evaluation {evaluation_id} cancelled successfully"
        )
...
async def simulate_evaluation(evaluation_id: str, request: EvaluationRequest):
    ...
    try:
        ...
        evaluation.average_episode_length = sum(r["length"] for r in episode_results) / len(episode_results)
        ...
        logger.info(f"Completed evaluation {evaluation_id}: avg_reward={evaluation.average_reward:.2f}, success_rate={evaluation.success_rate:.2%}")
