...
async def cmd_monitor(args) -> int:
    ...
    try:
        ...
        if args.save_report:
            ...
            report_file = Path(args.output_dir) / f"genesis_compatibility_{timestamp}.json"
        ...
        for suite_name, suite in results.items():
            ...
            if args.verbose:
                failed_tests = [test for test in suite.tests 
                              if test.actual_result and test.actual_result.value in ['fail', 'error']]
                if failed_tests:
...
Examples:
  ...
  %(prog)s benchmark                       # Run performance benchmarks
    ...
    status_parser = subparsers.add_parser(
        'status',
        help='Check Genesis installation status'
    )
    
    # Monitor command
    monitor_parser = subparsers.add_parser(
        'monitor',
        help='Run Genesis API compatibility monitoring'
    )
    monitor_parser.add_argument(
        '--save-report',
        action='store_true',
        help='Save detailed compatibility report'
    )
    
    # Test version command
    test_parser = subparsers.add_parser(
        'test-version',
        help='Run Genesis version compatibility tests'
    )
    test_parser.add_argument(
        '--version',
        help='Genesis version to test (default: current)'
    )
    test_parser.add_argument(
        '--suites',
        dest='test_suites',
    ...
    test_all_parser = subparsers.add_parser(
        'test-all',
        help='Run comprehensive Genesis compatibility testing'
    )
    
    # Benchmark command
    benchmark_parser = subparsers.add_parser(
        'benchmark',
        help='Run Genesis performance benchmarks'
    )
