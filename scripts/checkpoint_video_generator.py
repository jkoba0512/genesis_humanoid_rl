...
class CheckpointVideoGenerator:
    ...
    def generate_all_checkpoint_videos(self, checkpoint_dir: str, phase_name: str) -> List[str]:
    ...
    def create_checkpoint_video(self, model_path: str, timesteps: int, phase: str) -> Optional[str]:
        ...
        try:
            ...
            if positions:
                ...
                if len(positions) > 1:
                    distance_traveled = np.sqrt(np.sum((positions[-1, :2] - positions[0, :2])**2))
                    avg_height = np.mean(positions[:, 2])
                    max_distance = np.max([np.sqrt(np.sum((pos[:2] - positions[0, :2])**2)) for pos in positions])
                else:
            ...
            video_info = {
                ...
                "performance": {
                    ...
                    "episode_count": int(np.sum([r > 0 for r in rewards])) if rewards else 0,
            ...
            if video_path.exists():
                ...
                print(f"   🏆 Success: {'✅' if video_info['performance']['success'] else '❌'}")
    ...
    def create_comparison_video(self, video_paths: List[str], phase: str) -> Optional[str]:
        ...
        comparison_data = {
            ...
            "performance_summary": {
                ...
                "improvement": video_infos[-1]['performance']['distance_traveled'] - video_infos[0]['performance']['distance_traveled'],
        ...
        print(f"   📈 Distance improvement: {comparison_data['performance_summary']['improvement']:.2f}m")
        print(f"   🎯 Final distance: {comparison_data['performance_summary']['final_distance']:.2f}m")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate checkpoint evaluation videos")
    parser.add_argument("--config", type=str, required=True,
                       help="Path to training configuration file")
    parser.add_argument("--checkpoint-dir", type=str, required=True,
                       help="Path to checkpoint directory")
    parser.add_argument("--phase", type=str, required=True,
                       help="Training phase name")
