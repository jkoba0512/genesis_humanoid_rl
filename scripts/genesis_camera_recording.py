...
def record_genesis_video():
    ...
    print(f"📋 Recording plan: {len(phases)} phases, {total_steps} steps ({total_steps/60:.1f}s)")
        ...
        for phase_idx, phase in enumerate(phases):
            ...
            for step in range(phase["steps"]):
                ...
                if phase["name"] == "balance":
                    ...
                    if robot.n_dofs >= 20:
                        # Hip movements
                        action[6] = 0.2 * np.sin(t * 3) + 0.05 * np.sin(t * 9)  # Left hip
                        action[7] = -0.2 * np.sin(t * 3) - 0.05 * np.sin(t * 9)  # Right hip
                        
                        # Knee movements
                        action[12] = 0.15 * np.sin(t * 3 + np.pi/2) + 0.1  # Left knee
                        action[13] = 0.15 * np.sin(t * 3 - np.pi/2) + 0.1  # Right knee
                ...
                if step % 30 == 0:  # Every 0.5 seconds
                    ...
                    print(f"  Progress: {progress:.1f}% (Step {current_step}/{total_steps})")
