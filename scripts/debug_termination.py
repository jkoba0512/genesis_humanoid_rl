...
def debug_termination():
    ...
    if env.robot is not None:
        ...
        print(f"Initial robot quaternion: [{quat[0]:.3f}, {quat[1]:.3f}, {quat[2]:.3f}, {quat[3]:.3f}]")
        ...
        if quat_norm > 0:
            ...
            print(f"Uprightness check: rotation_magnitude={rotation_magnitude:.3f} <= 0.8 = {upright_ok}")
        else:
            upright_ok = False
        ...
        print(f"Bounds check: |{pos[0]:.3f}| <= 10.0 and |{pos[1]:.3f}| <= 5.0 = {bounds_ok}")
        ...
        if should_terminate:
            print("❌ Robot should terminate immediately after reset - this is the problem!")
        else:
    ...
    for step in range(5):
        ...
        print(f"Step {step}: reward={reward:.3f}, terminated={terminated}, pos=[{pos[0]:.3f}, {pos[1]:.3f}, {pos[2]:.3f}]")
