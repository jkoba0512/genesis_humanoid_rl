        ...
        for info in infos:
            if 'episode' in info:
                ...
                if 'curriculum' in info:
                    ...
                    # Log to tensorboard
                    self.logger.record("curriculum/stage", curr_info.get('current_stage', 'unknown'))
                    self.logger.record("curriculum/target_velocity", curr_info.get('target_velocity', 0.0))
                    self.logger.record("curriculum/recent_avg_reward", curr_info.get('recent_avg_reward', 0.0))
                    self.logger.record("curriculum/total_episodes", curr_info.get('total_episodes', 0))
                    
                    # Print stage changes
                    current_stage = curr_info.get('current_stage', 'unknown')
                    if not self.stage_changes or self.stage_changes[-1] != current_stage:
                        self.stage_changes.append(current_stage)
                        print(f"\n🎓 CURRICULUM STAGE: {current_stage}")
                        print(f"   Episode Reward: {episode_reward:.2f}")
                        print(f"   Episode Length: {episode_length}")
                        print(f"   Target Velocity: {curr_info.get('target_velocity', 0.0):.2f}")
        
        # Log periodic statistics
        if len(self.episode_rewards) > 0 and len(self.episode_rewards) % self.log_freq == 0:
            ...
            print(f"   Current Stage: {self.stage_changes[-1] if self.stage_changes else 'unknown'}")
...
def train_curriculum(config_path: str = None, render: bool = False):
    ...
    try:
        ...
        # Save final model
        final_model_path = os.path.join(training_config["model_dir"], "final_curriculum_model")
        ...
        if hasattr(env, 'save_curriculum_progress'):
            curriculum_path = os.path.join(training_config["model_dir"], "final_curriculum_progress.json")
        ...
        if hasattr(env, 'get_curriculum_info'):
            ...
            print(f"   Stage History: {len(final_info.get('stage_history', []))} completed stages")
        ...
        print(f"   Test episode: {steps} steps, {total_reward:.2f} reward, {total_reward/steps:.3f} avg")
        
    except KeyboardInterrupt:
        print("\n⏸️ Training interrupted by user")
        interrupt_model_path = os.path.join(training_config["model_dir"], "interrupted_curriculum_model")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Train humanoid with curriculum learning")
