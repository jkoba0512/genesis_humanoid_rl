...
class ProgressiveTrainingMonitor(BaseCallback):
    ...
    def _handle_checkpoint(self, timesteps: int):
        ...
        # Update status
        self._update_status({
            "checkpoints_completed": self.checkpoint_steps[:self.checkpoint_steps.index(timesteps) + 1],
    ...
    def _update_progress(self):
        ...
        eta = (elapsed / current_timesteps) * (total_timesteps - current_timesteps) if current_timesteps > 0 else 0
        
        print(f"📊 Progress: {progress:.1f}% ({current_timesteps:,}/{total_timesteps:,}) | "
    ...
    def _update_status(self, updates: Dict[str, Any]):
        ...
        try:
            ...
            status.update({
                ...
                "progress_percent": (self.num_timesteps / self.config['training']['total_timesteps']) * 100,
...
class VideoGenerator:
    ...
    def create_checkpoint_video(self, model_path: str, timesteps: int, phase: str) -> str:
        ...
        print(f"📊 Evaluation: {distance_traveled:.2f}m distance, {total_reward:.2f} reward")
...
def check_system_requirements():
    ...
    try:
        ...
        if gpus:
            gpu = gpus[0]
            print(f"🎮 GPU: {gpu.name} ({gpu.memoryTotal:.1f}MB total, {gpu.memoryFree:.1f}MB free)")
