...
def evaluate_model(model_path: str, n_episodes: int = 10, render: bool = False):
    ...
    print(f"Environment created - Obs: {env.observation_space.shape}, Action: {env.action_space.shape}")
...
def compare_models(model_paths: list, n_episodes: int = 5):
    ...
    for model_path in model_paths:
        if os.path.exists(model_path + ".zip"):
            mean_reward, std_reward = evaluate_model(model_path, n_episodes, render=False)
