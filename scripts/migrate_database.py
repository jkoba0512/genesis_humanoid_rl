...
from src.genesis_humanoid_rl.infrastructure.persistence.migration_manager import (
    DatabaseMigrationManager, MigrationError
)
...
def status_command(args):
    ...
    print(f"Optimized Schema: {'✅ Applied' if status['has_optimized_schema'] else '❌ Not Applied'}")
...
def migrate_command(args):
    ...
    try:
        ...
        print(f"🎯 Performance: {len(new_status['current_tables']) * 3} indexes created")
...
def validate_command(args):
    ...
    # Check table counts
    from src.genesis_humanoid_rl.infrastructure.persistence.schema_design import OptimizedSchemaDesign
...
def backup_command(args):
    ...
    if backup_status['migration_history']:
        print(f"📝 Migration history: {len(backup_status['migration_history'])} entries")
...
Examples:
  ...
  %(prog)s performance                     # Analyze performance
    ...
    perf_parser = subparsers.add_parser('performance', help='Analyze schema performance')
