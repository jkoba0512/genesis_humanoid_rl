...
class ProgressiveTrainingManager:
    ...
    def run_phase_training(self, phase: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            # Run training script
            cmd = [
                "python", str(self.scripts_dir / "progressive_training.py"),
    ...
    def generate_phase_videos(self, phase: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            cmd = [
                "python", str(self.scripts_dir / "checkpoint_video_generator.py"),
                "--config", str(config_path),
            ...
            self.log_message("🎥 Video generation simulation (actual generation would run here)")
    ...
    def check_phase_success(self, phase: Dict[str, Any], results: Dict[str, Any]) -> bool:
            ...
            if metric.startswith("min_"):
                ...
                if actual_metric in results:
                    if results[actual_metric] < threshold:
                        self.log_message(f"❌ {actual_metric} {results[actual_metric]:.1f} < {threshold} (threshold)")
                        success = False
                    else:
                        self.log_message(f"✅ {actual_metric} {results[actual_metric]:.1f} >= {threshold} (threshold)")
    ...
    def run_progressive_training(self, start_phase: Optional[str] = None, dry_run: bool = False) -> bool:
        ...
        for i in range(start_index, len(self.phases)):
            ...
            if success:
                ...
                if self.check_phase_success(phase, results):
                    ...
                    self.log_message(f"⚠️ Phase {phase['name']} completed but didn't meet success criteria")
        ...
        self.log_message(f"⏱️ Total time: {status.get('total_training_time_hours', 0):.2f}h")
...
def main():
    ...
    parser.add_argument("--start-phase", type=str, choices=["phase1", "phase2", "phase3"],
