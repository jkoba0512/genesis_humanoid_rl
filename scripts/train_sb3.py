...
def train(config_path: str = None, render: bool = False):
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=training_config["save_freq"] // env_config["n_envs"],  # Adjust for vectorized env
    ...
    try:
        ...
        # Save current model
        interrupt_model_path = os.path.join(training_config["model_dir"], "interrupted_model")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Train humanoid walking with Stable-Baselines3")
    parser.add_argument(
        "--config",
        type=str,
