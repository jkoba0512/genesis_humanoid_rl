...
def create_environment_manually(episode_length=200, simulation_fps=50, control_freq=20, target_velocity=1.0):
...
def main():
    parser = argparse.ArgumentParser(description="Record video of trained humanoid model")
    parser.add_argument("--model", default="./models/test_sb3/final_model",
                       help="Path to trained model")
    parser.add_argument("--output", default="trained_robot_demo.mp4",
    ...
    parser.add_argument("--camera-orbit", action="store_true",
                       help="Make camera orbit around robot")
    ...
    for step in range(args.steps):
        ...
        if args.camera_orbit:
            ...
            camera.set_pose(
                pos=(camera_x, camera_y, args.camera_height),
                lookat=(0.0, 0.0, 0.8)
            )
        ...
        if step % 50 == 0 and step > 0:
            ...
            fps = step / elapsed
            print(f"  Step {step}/{args.steps}: reward={reward:.3f}, total={total_reward:.2f}, {fps:.1f} steps/s")
