...
def create_sample_training_data():
    ...
    for step in range(0, total_steps, 100):
        ...
        writer.add_scalar('train/entropy_loss', -0.01 * (1 + 0.1 * np.random.randn()), step)
        writer.add_scalar('train/approx_kl', 0.02 * (1 + 0.2 * np.random.randn()), step)
        writer.add_scalar('train/clip_fraction', 0.1 * (1 + 0.3 * np.random.randn()), step)
        ...
        writer.add_scalar('custom/forward_distance', step / 100 * (1 + 0.1 * np.random.randn()), step)
        writer.add_scalar('custom/stability_score', min(1.0, step / 5000 + 0.1 * np.random.randn()), step)
