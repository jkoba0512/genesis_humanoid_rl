...
from genesis_humanoid_rl.config.training_config import get_default_config, load_config_from_dict
...
def create_agent(environment_spec: specs.EnvironmentSpec, agent_config) -> PPOHumanoidAgent:
...
def run_training(config_path: str = None, resume_from: str = None):
    ...
    print(f"{'Episode':<10} {'Steps':<10} {'Reward':<15} {'Avg Reward':<15} {'Episode Length':<15}")
    print("-" * 75)
    ...
    while total_steps < training_config.total_steps:
        ...
        if episode % training_config.log_interval == 0:
            print(f"{episode:<10} {total_steps:<10} {episode_reward:<15.2f} {avg_reward:<15.2f} {episode_length:<15}")
