...
class SystemMonitor:
    ...
    def print_status(self, status: Dict[str, Any], alerts: Dict[str, str]):
        ...
        # Memory
        mem = status['memory']
        mem_status = "🔴" if 'memory' in alerts and alerts['memory'] == 'CRITICAL' else \
                    "🟡" if 'memory' in alerts else "🟢"
        print(f"{mem_status} Memory: {mem['used_gb']:.1f}/{mem['total_gb']:.1f}GB "
        ...
        gpu = status['gpu']
        if 'error' not in gpu:
            gpu_status = "🔴" if 'gpu_memory' in alerts and alerts['gpu_memory'] == 'CRITICAL' else \
                        "🟡" if 'gpu_memory' in alerts else "🟢"
            print(f"{gpu_status} GPU: {gpu['name']} | "
        ...
        # Process
        proc = status['process']
        print(f"🔧 Process: {proc['memory_mb']:.0f}MB | "
    ...
    def monitor_continuously(self, interval_seconds: int = 30, max_duration_minutes: Optional[int] = None):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="System monitoring for progressive training")
    parser.add_argument("--check", action="store_true",
                       help="Check training readiness")
    ...
    parser.add_argument("--log-file", type=str, default="./system_monitor.log",
                       help="Log file path")
